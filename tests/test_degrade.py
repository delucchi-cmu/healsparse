...
class DegradeMapTestCase(unittest.TestCase):
    ...
    def test_degrade_map_float(self):
        ...
        sparse_map = healsparse.HealSparseMap(
            healpix_map=full_map, nside_coverage=nside_coverage, nside_sparse=nside_map
        )
    ...
    def test_degrade_map_int(self):
        ...
        deg_map = hp.ud_grade(
            full_map.astype(np.float64), nside_out=nside_new, order_in="NESTED", order_out="NESTED"
        )
    ...
    def test_degrade_map_recarray(self):
        ...
        testing.assert_almost_equal(
            new_map.get_values_pos(ra, dec, lonlat=True)["col1"], hpmap_col1[ipnest_test]
        )
        testing.assert_almost_equal(
            new_map.get_values_pos(ra, dec, lonlat=True)["col2"], hpmap_col2[ipnest_test]
        )
        testing.assert_almost_equal(
            new_map.get_values_pos(ra, dec, lonlat=True)["col3"], hpmap_col3[ipnest_test]
        )
        ...
        testing.assert_almost_equal(
            new_map2.get_values_pos(ra, dec, lonlat=True)["col1"], hpmap_col1[ipnest_test]
        )
        testing.assert_almost_equal(
            new_map2.get_values_pos(ra, dec, lonlat=True)["col2"], hpmap_col2[ipnest_test]
        )
        testing.assert_almost_equal(
            new_map2.get_values_pos(ra, dec, lonlat=True)["col3"], hpmap_col3[ipnest_test]
        )
    ...
    def test_degrade_widemask_or(self):
        ...
        pixel2_all = np.unique(
            np.right_shift(pixel, healsparse.utils._compute_bitshift(nside_map2, nside_map))
        )
    ...
    def test_degrade_widemask_and(self):
        ...
        pixel2_bad = np.unique(
            np.right_shift(pixel[512:], healsparse.utils._compute_bitshift(nside_map2, nside_map))
        )
    ...
    def test_degrade_int_or(self):
        ...
        nside_map2 = 64
        sparse_map = healsparse.HealSparseMap.make_empty(nside_coverage, nside_map, np.int64)
        sparse_map_or = healsparse.HealSparseMap.make_empty(nside_coverage, nside_map2, np.int64)
        # Fill some pixels in the "high-resolution" map
        pixel = np.arange(4000, 8000)
    ...
    def test_degrade_int_and(self):
        ...
        nside_map2 = 64
        sparse_map = healsparse.HealSparseMap.make_empty(nside_coverage, nside_map, np.int64)
        sparse_map_and = healsparse.HealSparseMap.make_empty(nside_coverage, nside_map2, np.int64)
        # Fill some pixels in the "high-resolution" map
        pixel = np.arange(4000, 8000)
        sparse_map.update_values_pix(pixel, pixel)
        ...
        # Check which pixels will be full in the "low-resolution" map and fill them
        pixel2 = np.unique(np.right_shift(pixel, healsparse.utils._compute_bitshift(nside_map2, nside_map)))
        px2val = np.arange(4000, 8000, (nside_map // nside_map2) ** 2)
    ...
    def test_degrade_map_float_prod(self):
        ...
        nside_map = 1024
        nside_new = 512
        full_map = np.full(hpg.nside_to_npixel(nside_map), 2.0)
        # Generate sparse map
        ...
        deg_map = np.full(hpg.nside_to_npixel(nside_new), 2.0**4)
        # Degrade sparse map and compare to original
    ...
    def test_degrade_map_int_prod(self):
        ...
        sparse_map = healsparse.HealSparseMap(
            healpix_map=full_map, nside_coverage=nside_coverage, nside_sparse=nside_map, sentinel=0
        )
    ...
    def test_degrade_map_float_sum(self):
        ...
        nside_map = 1024
        nside_new = 512
        full_map = np.full(hpg.nside_to_npixel(nside_map), 1.0)
        # Generate sparse map
        ...
        deg_map = np.full(hpg.nside_to_npixel(nside_new), 4.0)
        # Degrade sparse map and compare to original
    ...
    def test_degrade_map_wmean(self):
        ...
        weights = healsparse.HealSparseMap(
            healpix_map=weights, nside_coverage=nside_coverage, nside_sparse=nside_map
        )
        ...
        new_map2 = healsparse.HealSparseMap.read(
            fname, degrade_nside=nside_new, reduction="wmean", weightfile=fname_weight
        )
    ...
    def test_degrade_map_int_wmean(self):
        ...
        weights = healsparse.HealSparseMap(
            healpix_map=np.ones(len(full_map)), nside_coverage=nside_coverage, nside_sparse=nside_map
        )
        # Degrade original HEALPix map
    ...
    def test_degrade_widemask_wmean(self):
        ...
        testing.assert_raises(
            NotImplementedError, sparse_map.degrade, nside_out=nside_out, reduction="wmean", weights=weights
        )
    ...
    def test_degrade_lowres_wide(self):
        ...
        for method in ["and", "or"]:
            ...
            sparse_map2 = healsparse.HealSparseMap.make_empty(
                nside_out, nside_map, WIDE_MASK, wide_mask_maxbits=7
            )
    ...
    def test_degrade_map_bool(self):
        ...
        deg_map = hp.ud_grade(test_map, nside_out=nside_new, order_in="NESTED", order_out="NESTED")
