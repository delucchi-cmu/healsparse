...
class ValidAreaTestCase(unittest.TestCase):
    def test_valid_area(self):
        ...
        n_rand = 1000
        r_indices = np.random.choice(np.arange(hpg.nside_to_npixel(nside_map)), size=n_rand, replace=False)
        ...
        for dt in [np.float64, np.int64]:
            ...
            testing.assert_equal(
                sparse_map.get_valid_area(), n_rand * hpg.nside_to_pixel_area(nside_map, degrees=True)
            )
