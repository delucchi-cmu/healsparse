...
class FitsShimTestCase(unittest.TestCase):
    def test_read_header(self):
        ...
        header = healsparse.fits_shim._make_header({"AA": 0, "BB": 1.0, "CC": "test"})
                ...
                for ext in exts:
                    ...
                    if t == "recarray":
                        ...
                        for n in dtype.names:
                            self.assertEqual(var_in[n][0].__class__, var_out[n][0].__class__)
                    else:
    ...
    def write_testfile_unused(self, filename, data0, data1, header):
        ...
        _header["EXTNAME"] = "COV"
        healsparse.fits_shim.fits.writeto(filename, data0, header=_header)
        _header["EXTNAME"] = "SPARSE"
        healsparse.fits_shim.fits.append(filename, data1, header=_header, overwrite=False)
    ...
    def write_testfile(self, filename, data0, data1, header):
        ...
        """
        if healsparse.fits_shim.use_fitsio:
            healsparse.fits_shim.fitsio.write(filename, data0, header=header, extname="COV")
            healsparse.fits_shim.fitsio.write(filename, data1, header=header, extname="SPARSE")
        else:
            _header = healsparse.fits_shim._make_header(header)
            _header["EXTNAME"] = "COV"
            healsparse.fits_shim.fits.writeto(filename, data0, header=_header)
