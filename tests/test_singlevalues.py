...
class SingleValuesTestCase(unittest.TestCase):
    def test_singlevalue_int(self):
        ...
        # Test floating point
        self.assertRaises(ValueError, m.update_values_pix, np.arange(100), 1.0)

        # Test floating point ndarray, length 1
        self.assertRaises(ValueError, m.update_values_pix, np.arange(100), np.ones(1, dtype=np.float32))

        # Test numpy floating point
        self.assertRaises(ValueError, m.update_values_pix, np.arange(100), np.ones(1, dtype=np.float32)[0])
    ...
    def test_singlevalue_float(self):
        ...
        self.assertRaises(ValueError, m.update_values_pix, np.arange(100), np.ones(1, dtype=np.int32))

        # Test numpy int
        self.assertRaises(ValueError, m.update_values_pix, np.arange(100), np.ones(1, dtype=np.int32)[0])
    ...
    def test_singlevalue_widemask(self):
        ...
        self.assertRaises(ValueError, m.update_values_pix, np.arange(100), 1)
        self.assertRaises(ValueError, m.update_values_pix, np.arange(100), np.zeros(3, dtype=np.int8))
        self.assertRaises(ValueError, m.update_values_pix, np.arange(100), np.zeros(3, dtype=np.int16))
