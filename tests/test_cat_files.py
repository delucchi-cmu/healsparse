...
class CatFilesTestCase(unittest.TestCase):
    def test_cat_maps(self):
        ...
        for t in ["array", "recarray", "widemask"]:
            ...
            combined_map = healsparse.HealSparseMap.make_empty_like(map1, nside_coverage=nside_coverage_out)
            ...
            for in_mem in [False, True]:
                ...
                if not healsparse.fits_shim.use_fitsio and not in_mem:
                    ...
                    cat_healsparse_files(
                        file_list, outfile, in_memory=in_mem, nside_coverage_out=nside_coverage_out
                    )
                    ...
                    if t == "recarray":
                        for col in map_test.dtype.names:
                            testing.assert_array_almost_equal(map_test[:][col], combined_map[:][col])
                    else:
    ...
    def test_cat_maps_overlap(self):
        ...
        for t in ["array"]:  # 'recarray', 'widemask']:
            ...
            # This should have the same nside_coverage as map2
            combined_map = healsparse.HealSparseMap.make_empty_like(
                map1, nside_coverage=nside_coverage_out, sentinel=sentinel
            )
            if combined_map.is_integer_map:
                ...
                combined_map[map2.valid_pixels[overlap]] = (
                    map2[map2.valid_pixels[overlap]] | combined_map[map2.valid_pixels[overlap]]
                )
                ...
                combined_map[map3.valid_pixels[overlap]] = (
                    map3[map3.valid_pixels[overlap]] | combined_map[map3.valid_pixels[overlap]]
                )
                ...
                if not healsparse.fits_shim.use_fitsio and not in_mem:
                    ...
                    if not combined_map.is_integer_map:
                        ...
                        testing.assert_array_almost_equal(
                            map_test[map_test.valid_pixels], combined_map[combined_map.valid_pixels]
                        )
