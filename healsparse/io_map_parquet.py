...
from .utils import WIDE_MASK, _compute_bitshift, check_sentinel, is_integer_value
...
def _read_map_parquet(
    ...
    if md["healsparse::primary"] != "":
        ...
        dtype = [(name, to_numpy_dtype(schema.field(name).type)) for name in columns]
    ...
    if is_rec_array:
        ...
        if is_wide_mask:
            sparse_map = sparse_map.reshape((sparse_map.size // wmult, wmult)).astype(WIDE_MASK)
        elif is_bit_packed:
            sparse_map = _PackedBoolArray(data_buffer=sparse_map)

    healsparse_map = healsparse_class(
        cov_map=cov_map, sparse_map=sparse_map, nside_sparse=nside_sparse, primary=primary, sentinel=sentinel
    )
...
def _write_map_parquet(hsp_map, filepath, clobber=False, nside_io=4):
    ...
    if not hsp_map.is_rec_array:
        schema = pa.schema(
            [("cov_pix", pa.from_numpy_dtype(np.int32)), ("sparse", pa.from_numpy_dtype(sparse_map.dtype))],
            metadata=metadata,
        )
    else:
        type_list = [(name, pa.from_numpy_dtype(hsp_map.dtype[name].type)) for name in hsp_map.dtype.names]
    ...
    cov_index_map_temp = (
        cov_map[:]
        + np.arange(hpg.nside_to_npixel(hsp_map.nside_coverage), dtype=np.int64) * cov_map.nfine_per_cov
    )
    ...
    for ctr, (cpix_io, cpix) in enumerate(zip(cov_pixels_io, cov_pixels)):
        ...
        sparsepix = sparse_map[
            cov_index_map_temp[cpix]
            * wmult
            // wdiv : (cov_index_map_temp[cpix] + cov_map.nfine_per_cov)
            * wmult
            // wdiv
        ]
        cpix_arr[:] = cpix
        if not hsp_map.is_rec_array:
            arrays = [pa.array(cpix_arr), pa.array(sparsepix)]
        else:
            arrays = [pa.array(sparsepix[name]) for name in hsp_map.dtype.names]
    ...
    tab = pa.Table.from_pydict({"cov_pix": pa.array(cov_pixels), "row_group": pa.array(row_groups)})
