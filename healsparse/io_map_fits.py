...
from .utils import WIDE_MASK, _compute_bitshift, is_integer_value, reduce_array
    ...
    if "PIXTYPE" in hdr and hdr["PIXTYPE"].rstrip() == "HEALPIX":
        ...
        if weightfile is not None and degrade_nside is not None:
            raise NotImplementedError(
                "Cannot specify a weightfile with degrade-on-read " "with a healpix map input."
            )
        ...
        if hdr["ORDERING"].rstrip() == "NUNIQ":
            ...
            healsparse_map = healsparse_class.make_empty(
                nside_coverage, hdr["NSIDE"], data[0][signal_column].dtype.type, sentinel=sentinel
            )
            ...
            healsparse_map = healsparse_class(healpix_map=data, nside_coverage=nside_coverage, nest=True)
        else:
        ...
        if degrade_nside is None:
            cov_map, sparse_map, nside_sparse, primary, sentinel = _read_healsparse_fits_file(
                filename, pixels=pixels
            )

            if "WIDEMASK" in hdr and hdr["WIDEMASK"]:
                sparse_map = sparse_map.reshape((sparse_map.size // hdr["WWIDTH"], hdr["WWIDTH"])).astype(
                    WIDE_MASK
                )
        else:
            # Read with degrade-on-read code
            cov_map, sparse_map, nside_sparse, primary, sentinel = _read_healsparse_fits_file_and_degrade(
                filename, pixels, degrade_nside, reduction, weightfile
            )

        healsparse_map = healsparse_class(
            cov_map=cov_map,
        ...
        if hdr["ORDERING"] != "NUNIQ":
            raise RuntimeError(
                "MOC file %s has %s ordering; only NUNIQ supported by healsparse."
...
def _read_moc_fits(healsparse_class, filename, nside_coverage):
    ...
    healsparse_map = healsparse_class.make_empty(nside_coverage, 2**max_order, dtype=bool)
    ...
    for uniq_order, uniq_index in zip(order, index):
        pixel_arrays.append(
            np.left_shift(uniq_index, 2 * (max_order - uniq_order)) + np.arange(4 ** (max_order - uniq_order))
        )
...
def _read_healsparse_fits_file(filename, pixels=None):
    ...
    if pixels is None:
        ...
        with HealSparseFits(filename) as fits:
            ...
            # but it does work.
            sparse_map = np.zeros(
                (_pixels.size + 1) * cov_map.nfine_per_cov * wmult // wdiv, dtype=fits.get_ext_dtype("SPARSE")
            )
            ...
            # if reshaped:
            #     print("About to read ... ")
            sparse_map[0 : cov_map.nfine_per_cov * wmult // wdiv] = fits.read_ext_data(
                "SPARSE", row_range=row_range, col_range=col_range
            ).ravel()
            # And read in the pixels
            for i, pix in enumerate(_pixels):
                if not reshaped:
                    row_range = [
                        cov_index_map_temp[pix] * wmult // wdiv,
                ...
                sparse_map[
                    ...
                    * wmult
                    // wdiv
                ] = fits.read_ext_data("SPARSE", row_range=row_range, col_range=col_range).ravel()
...
def _read_healsparse_fits_file_and_degrade(filename, pixels, nside_out, reduction, weightfile):
    ...
    if weightfile is not None and reduction == "wmean":
        ...
        if not np.all(np.in1d(_pixels, cov_pix_weight)):
            raise ValueError(
                "The weightfile %s must have coverage in all the " "pixels to read." % (weightfile)
            )
        use_weightfile = True
    ...
    cov_map_out = HealSparseCoverage.make_from_pixels(nside_coverage, nside_out, _pixels)
    # This is the map without the offset
    cov_index_out_temp = (
        cov_map_out[:]
        + np.arange(hpg.nside_to_npixel(nside_coverage), dtype=np.int64) * cov_map_out.nfine_per_cov
    )
    with HealSparseFits(filename) as fits:
        s_hdr = fits.read_ext_header("SPARSE")
        ...
        if nside_out >= nside_sparse:
            raise ValueError(
                "Degrade nside (%d) is not smaller than sparse nside (%d)" % (nside_out, nside_sparse)
            )
        ...
        if use_weightfile:
            ...
            if (
                s_hdr_weight["NSIDE"] != nside_sparse
                or not fits.ext_is_image("SPARSE")
                or "WIDEMASK" in s_hdr_weight
                or is_integer_value(testval)
            ):
                wfits.close()
                raise ValueError("Weights must be a floating-point map with same " "nside as map to degrade.")
        ...
        if is_wide_mask:
            if reduction not in ["and", "or"]:
                if use_weightfile:
                    wfits.close()
                raise NotImplementedError(
                    "Cannot degrade a wide_mask map with any operation " "except for and/or"
                )
            sentinel_out = sentinel
            dtype_out = dtype
            sparse_map_out = np.zeros(((_pixels.size + 1) * nfine_per_cov_out, wmult), dtype=dtype_out)
        elif is_rec_array:
            dtype_out = []
            ...
            sentinel_out = sentinel
            dtype_out = dtype
            sparse_map_out = np.full((_pixels.size + 1) * nfine_per_cov_out, sentinel_out, dtype=dtype_out)
        else:
        ...
        for i, pix in enumerate(_pixels):
            ...
            if use_weightfile:
                ...
                weight_values = weight_values.reshape((1, (nside_out // nside_coverage) ** 2, -1))
            else:
                weight_values = None
            ...
            sparse_map_out[cov_index_out_temp[pix] : cov_index_out_temp[pix] + cov_map_out.nfine_per_cov] = (
                aux
            )
...
def _write_map_fits(hsp_map, filename, clobber=False, nocompress=False):
    ...
    if hsp_map._is_wide_mask:
        ...
        _write_filename(
            ...
            s_hdr,
            hsp_map._cov_map[:],
            hsp_map._sparse_map.data_array,
            compress=not nocompress,
