...
from .utils import WIDE_MASK, WIDE_NBIT, _compute_bitshift


def cat_healsparse_files(
    file_list,
    outfile,
    check_overlap=False,
    clobber=False,
    in_memory=False,
    nside_coverage_out=None,
    or_overlap=False,
):
    """
    ...
    for i, f in enumerate(file_list):
        ...
        cov_index_map = (
            cov_map[:]
            + np.arange(hpg.nside_to_npixel(cov_map.nside_coverage), dtype=np.int64) * cov_map.nfine_per_cov
        )
        ...
        if cov_mask_summary is None:
            ...
            cov_mask_summary = np.zeros(
                (len(file_list), hpg.nside_to_npixel(nside_coverage_out)), dtype=np.bool_
            )
    ...
    with HealSparseFits(file_list[0]) as fits:
        ...
        sparse_stub = fits.read_ext_data("SPARSE", row_range=[0, cov_map.nfine_per_cov * wmult])
    ...
    if not in_memory:
        ...
        stub_map = HealSparseMap(
            cov_map=cov_map,
            sparse_map=sparse_stub,
    ...
    for pix in cov_pix:
        ...
        for index in u_cov_pix:
            if nside_coverage_maps[index] == nside_coverage_out:
                ...
                in_map = _read_partial_sparsemap(
                    fitses[index],
                    cov_map_nfine_per_covs[index],
                    wmult,
                    cov_index_maps[index],
                    np.array([pix]),
                    sparse_stub.dtype,
                    wide_mask_maxbits,
                    nside_coverage_maps[index],
                    nside_sparse,
                ...
                _pixels = np.left_shift(pix, bit_shift_covs[index]) + np.arange(
                    2 ** bit_shift_covs[index], dtype=np.int32
                )
            ...
            if check_overlap:
                if np.any(sparse_map[valid_pixels] != sparse_map._sentinel):
                    if not sparse_map.is_integer_map or not or_overlap:
                        outfits.close()
                        raise RuntimeError(
                            "Map %s has pixels that were already set in coverage pixel %d"
                            % (file_list[index], pix)
                        )
                    else:
...
def _read_partial_sparsemap(
    ...
    if len(pixels) == 1:
        ...
        if sparse_map_temp_input is None:
            sparse_map_temp = np.zeros(2 * nfine_per_cov * wmult, dtype=dtype)
            row_range = [0, nfine_per_cov * wmult]
            sparse_map_temp[0 : nfine_per_cov * wmult] = fits.read_ext_data("SPARSE", row_range=row_range)
        else:
            sparse_map_temp = sparse_map_temp_input

        row_range = [
            cov_index_map_temp[pixels[0]] * wmult,
            (cov_index_map_temp[pixels[0]] + nfine_per_cov) * wmult,
        ]
        sparse_map_temp[nfine_per_cov * wmult : 2 * nfine_per_cov * wmult] = fits.read_ext_data(
            "SPARSE", row_range=row_range
        )
    else:
        ...
        sparse_map_temp = np.zeros((sub.size + 1) * nfine_per_cov * wmult, dtype=dtype)
        row_range = [0, nfine_per_cov * wmult]
        sparse_map_temp[0 : nfine_per_cov * wmult] = fits.read_ext_data("SPARSE", row_range=row_range)

        for i, sub_pix in enumerate(cov_pix_temp[sub]):
            row_range = [
                cov_index_map_temp[sub_pix] * wmult,
                (cov_index_map_temp[sub_pix] + nfine_per_cov) * wmult,
            ]
            sparse_map_temp[(i + 1) * nfine_per_cov * wmult : (i + 2) * nfine_per_cov * wmult] = (
                fits.read_ext_data("SPARSE", row_range=row_range)
            )

    if wide_mask_maxbits is not None:
        sparse_map_temp = sparse_map_temp.reshape((sparse_map_temp.size // wmult, wmult)).astype(WIDE_MASK)

    cov_map_temp = HealSparseCoverage.make_from_pixels(nside_coverage, nside_sparse, pixels)
    partial_map = HealSparseMap(
